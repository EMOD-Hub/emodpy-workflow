...
for anything, e.g., calculations, notes, etc.
...
Scaling or Non-scaling. Scaling means the data channel scales with population (e.g. a population count) and
Non-scaling means it does not (e.g. a population fraction).
...
* two_sigma: 2 standard errors of count-type data. **Delete** from "Additional columns" if using scaling (percent) data.
* effective_count: Number of independent measurements (people). **Delete** from "Additional columns" if using count data.
...
All fractional data (e.g. Prevalence) must be between 0.0 and 1.0 (not percent).
